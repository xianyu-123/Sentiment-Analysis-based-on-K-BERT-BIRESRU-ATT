# -*- encoding:utf-8 -*-
"""
  This script provides an k-BERT and Bidirectional Residual Recurrent Unit res exmaple for classification.
"""
import sys
import torch
import json
...
class BertClassifier(nn.Module):
    def __init__(self, args, model):
        ...
        # add sru
        self.sru = SRU(input_size=args.hidden_size, hidden_size=args.hidden_size//2, bidirectional=True, num_layers=2,layer_norm=False)
        ...
        # TODO atten_w_ori
        self.w_omega = nn.Parameter(torch.Tensor(self.hidden_size, self.hidden_size).cuda())
        self.u_omega = nn.Parameter(torch.Tensor(self.hidden_size,1).cuda())
        nn.init.uniform_(self.w_omega,-0.1,0.1)
        nn.init.uniform_(self.u_omega,-0.1,0.1)
        print("[BertClassifier] use visible_matrix: {}".format(self.use_vm))

    def atten_net(self, lstm_out):
        # batch*size, seq_len,2*num_hiddens
        u = torch.tanh(torch.matmul(lstm_out,self.w_omega))
        att = torch.matmul(u, self.u_omega)
        att_score = F.softmax(att, dim=1)
        score_x = lstm_out * att_score
        context = torch.sum(score_x, dim=1)
        return context

    def biresruu(self,input_tensor):
        sru_out,h_z = self.sru(input_tensor)
        res_out = gelu(input_tensor)
        resru_out = torch.cat((sru_out, res_out), dim=1)
        return resru_out
